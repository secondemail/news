...
const CategoryContainer = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      let resSubCat = await axios.get(`https://my-json-server.typicode.com/secondemail/news/subCategory?id=${id}`);
      if (resSubCat) {
        setSubCatData(resSubCat.data);
      }
      let resNews = await axios.get(`https://my-json-server.typicode.com/secondemail/news/news?subCatId=${id}`);
      if (resNews) {
