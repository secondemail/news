...
const AddNews = () => {
  ...
  const handelselect = async(e) => {
    if (e.target.value !=="0") {
      let ressub = await axios.get(`https://my-json-server.typicode.com/secondemail/news/subCategory?catId=${e.target.value}`)
  ...
  const catSubmit = async (values) => {
    ...
    if (values.catId != "0" && values.catId !== "" &&values.subCatId != "0" && values.subCatId !== "" && images != "" && userId !="") {
      let res = await axios.post(`https://my-json-server.typicode.com/secondemail/news/news`, {
        title: values.title,
