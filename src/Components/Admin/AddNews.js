...
const AddNews = () => {
  ...
  useEffect(() => {
    ...
    widgetRef.current = cloudinaryRef.current.createUploadWidget({
        ...
        multiple: false,
        clientAllowedFormats: ["jpg","jpeg","png","jfif"]
  ...
  const handelselect = async(e) => {
    if (e.target.value !=="0") {
      let ressub = await axios.get(`http://localhost:3000/subCategory?catId=${e.target.value}`)
  ...
  const catSubmit = async (values) => {
    setIsPress(true);
    setLoading(true)
    if (values.catId != "0" && values.catId !== "" &&values.subCatId != "0" && values.subCatId !== "" && images != "") {
      let res = await axios.post(`http://localhost:3000/news`, {
        title: values.title,
